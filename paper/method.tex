...
The split deliberately exposes different stages of the residual hierarchy. The first route contains the largest coarse reconstruction terms, whereas the later routes encode progressively finer residual corrections. Consequently, the routes need not have equal standalone semantic fidelity. Our hypothesis is instead operational: their different collision patterns and query-conditioned decoding errors can provide complementary retrieval paths.
...
\subsubsection{Beam-conditional route posterior}
...
\label{eq:route-posterior}
    \widehat{p}_{v,B}(r\mid q)
    =
    \frac{\exp s_v(r\mid q)}
         {\sum_{r'\in\mathcal{B}_v(q)}
          \exp s_v(r'\mid q)}.
\end{equation}
Equation~\ref{eq:route-posterior} is a truncated distribution conditioned on the top-$B$ returned routes; it is not the full posterior over the complete trie.

We distribute each route's mass uniformly among the documents in its posting list:
\begin{equation}
\label{eq:view-doc-posterior}
    \widehat{p}_{v,B}(d\mid q)
    =
    \sum_{\substack{r\in\mathcal{B}_v(q):\\d\in\mathcal{P}_v(r)}}
    \frac{\widehat{p}_{v,B}(r\mid q)}
         {|\mathcal{P}_v(r)|}.
\end{equation}
This realizes a simple probability model in which the route posterior is known but the owner of a colliding route is otherwise unobserved. It also prevents a large posting list from assigning the full route score to every owner.

Finally, we average document evidence across views:
\begin{equation}
\label{eq:beam-posterior-fusion}
    S_{\mathrm{BP}}(d,q)
    =\frac{1}{V}\sum_{v=0}^{V-1}
      \widehat{p}_{v,B}(d\mid q).
\end{equation}
Documents are ranked in descending order of $S_{\mathrm{BP}}$; ties are broken by the best route rank and then the document index. This fusion is parameter-free, operates after route-to-document expansion, and assigns equal prior weight to each decoding view.

\subsection{Alternative Document-Level Fusion Rules}
\label{sec:fusion-ablations}

We retain three alternative families to separate candidate complementarity from the effect of score aggregation. These are baselines or upper bounds rather than the primary inference rule in Equation~\ref{eq:beam-posterior-fusion}.
...
\begin{equation}
    ...
    =\sum_{v=0}^{V-1}
      ...
      \right].
\end{equation}
Ties are resolved by the best route rank across views and then the document index. This baseline tests whether repeated retrieval alone explains the gain, but discards route confidence and collision size.

\paragraph{Validation-calibrated score fusion.}
For each view, we fit a temperature $T_v>0$ and bias $b_v$ on validation candidates:
\begin{equation}
    g_v(s)=s/T_v+b_v.
\end{equation}
